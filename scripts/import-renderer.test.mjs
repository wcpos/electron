...
for (const hasIndex of [true, false]) {
  test(hasIndex ? "replaces dist with the renderer export" : "rejects an export without index.html", (t) => {
    const root = mkdtempSync(join(tmpdir(), "wcpos-import-renderer-"));
    t.after(() => rmSync(root, { recursive: true, force: true }));
    mkdirSync(join(root, "scripts"));
    copyFileSync(new URL("./import-renderer.mjs", import.meta.url), join(root, "scripts/import-renderer.mjs"));
    mkdirSync(join(root, "export/assets"), { recursive: true });
    writeFileSync(join(root, "export/assets/app.js"), "renderer");
    if (hasIndex) writeFileSync(join(root, "export/index.html"), "<html>renderer</html>");
    mkdirSync(join(root, "dist"));
    writeFileSync(join(root, "dist/stale.txt"), "old");

    const result = spawnSync(process.execPath, [join(root, "scripts/import-renderer.mjs"), "export"], { cwd: root, encoding: "utf8" });
